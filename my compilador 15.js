  ...
  while (i < lineas.length && pasos < MAX_PASOS) {
    ...
    if (l.startsWith("leer")) {
      ...
      await new Promise(resolve => {
        rl.question(`Ingrese valor para ${variable}: `, respuesta => {
    ...
    // asignaciones (incluye arrays)
    if (l.includes("=")) {
      try { new Function("v", "with(v){ " + l + "}")(vars); }
      catch (e) { console.log("Error asignación:", e.message); }
      i++; continue;
    }
